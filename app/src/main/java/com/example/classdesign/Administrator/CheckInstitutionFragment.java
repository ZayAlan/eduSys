...
public class CheckInstitutionFragment extends Fragment {

    private View view;

    private List<Institution> courseList = new ArrayList<>();
    private int removePosition;

    private CheckInsItemAdapter courseAdapter;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        view = inflater.inflate(R.layout.checkinstitutionfragment,container,false);
        initCourseList();
        RecyclerView recyclerView = view.findViewById(R.id.check_institution_recyclerView);
        GridLayoutManager layoutManager = new GridLayoutManager(getContext(),1);
        recyclerView.setLayoutManager(layoutManager);
        courseAdapter = new CheckInsItemAdapter(courseList);
        ...
        courseAdapter.setOnItemClickListener(new CheckInsItemAdapter.OnItemClickListener() {
            @Override
    ...
    private Runnable removeRunnable = new Runnable() {
        @Override
        public void run() {
            Institution removeCourse = courseList.get(removePosition);
            ...
            Request request = new Request.Builder()
                    .url(MyUtilities.MYURL+"/user/allowEduInstitution")
                    .post(requestBody)
                    .build();
    ...
    private Runnable disagreeRunnable = new Runnable() {
        ...
        public void run() {
            ...
            RequestBody requestBody = FormBody.create(MediaType.parse("application/json; charset=utf-8"), jsonData);
            Request request = new Request.Builder()
                    .url(MyUtilities.MYURL+"/user/disallowEduInstitution")
                    .post(requestBody)
                    .build();
            Response response = null;
            try {
    ...
    private Runnable initCourseRunnable = new Runnable() {
        ...
        public void run() {
            ...
            OkHttpClient client = new OkHttpClient();
            Request request = new Request.Builder()
                    .url(MyUtilities.MYURL+"/user/showUncheckedEduInstitution")
                    .build();
            Response response = null;
            try {
                response = client.newCall(request).execute();
                String responseData = response.body().string();
                Gson gson = new Gson();
                courseList = gson.fromJson(responseData,new TypeToken<List<Institution>>(){}.getType());
                refreshHandler.sendMessage(refreshHandler.obtainMessage());
            } catch (IOException e) {
                Toast.makeText(getActivity(), "can't connected!", Toast.LENGTH_SHORT).show();
                e.printStackTrace();
