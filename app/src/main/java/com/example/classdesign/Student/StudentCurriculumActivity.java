...
import com.example.classdesign.CarItem;
...
import com.example.classdesign.TimeTableAdapter;
...
public class StudentCurriculumActivity extends AppCompatActivity {

    private List<CarItem> courseList = new ArrayList<>();
    private int removePosition;

    private TimeTableAdapter courseAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Toolbar toolbar = findViewById(R.id.toolbar_student_course);
        ...
        courseAdapter = new TimeTableAdapter(courseList);
        ...
        courseAdapter.setOnItemClickListener(new TimeTableAdapter.OnItemClickListener() {
            @Override
    ...
    private Runnable removeRunnable = new Runnable() {
        @Override
        public void run() {
            CarItem removeCourse = courseList.get(removePosition);
            ...
            Request request = new Request.Builder()
                    //待更改
                    .url(MyUtilities.MYURL+"/course/deleteTimetable")
                    .post(requestBody)
                    .build();
            ...
            try {
                ...
                Toast.makeText(StudentCurriculumActivity.this, "can't connected!", Toast.LENGTH_SHORT).show();
                e.printStackTrace();
            }
        }
    };
    ...
    private Runnable initCourseRunnable = new Runnable() {
        ...
        public void run() {
            //                发送json数据
            OkHttpClient client = new OkHttpClient();
            Parent enterParent = MyUtilities.getEnterParent();
            Gson sendGson = new Gson();
            String jsonData = sendGson.toJson(enterParent);
            RequestBody requestBody = FormBody.create(MediaType.parse("application/json; charset=utf-8"), jsonData);
            Request request = new Request.Builder()
                    .url(MyUtilities.MYURL+"/course/showParentTimetable")
                    .post(requestBody)
                    .build();
            Response response = null;
            try {
                ...
                String responseData = response.body().string();
                Gson gson = new Gson();
                courseList = gson.fromJson(responseData,new TypeToken<List<CarItem>>(){}.getType());
                refreshHandler.sendMessage(refreshHandler.obtainMessage());
            } catch (IOException e) {
                Toast.makeText(StudentCurriculumActivity.this, "can't connected!", Toast.LENGTH_SHORT).show();
                e.printStackTrace();
